...
{
	/// <summary>
	/// Summary description for clsObervacoesFaturaProforma.
	/// </summary>
	public class clsObservacoesFaturaProforma : clsObservacoes
	{
		#region Atributos
		...
		public clsObservacoesFaturaProforma(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string strIdPE): base(ref tratadorErro, ref ConnectionDB,EnderecoExecutavel,nIdExportador)
		{
			m_strIdPE = strIdPE;
			carregaTypDatSet();
			carregaDadosBD();
			m_strCaption = "Observações da Proforma";
		}
		#endregion
		...
		protected override void carregaDadosBDEspecifico()
		{
			try
			{
				...
				if (m_typDatSetTbFaturasProformas.tbFaturasProformas.Rows.Count > 0)
				{
					...
					if (!dtrwRowTbFaturasProformas.IsmstrObservacaoNull())
						m_strObservacoes = dtrwRowTbFaturasProformas.mstrObservacao;
				}
			} 
			catch (Exception err)
			{
			...
			try
			{
				...
				if (m_typDatSetTbFaturasProformas.tbFaturasProformas.Rows.Count > 0)
				{
					...
					if (dtrwRowTbFaturasProformas != null)
						dtrwRowTbFaturasProformas.mstrObservacao = m_strObservacoes;
				}
