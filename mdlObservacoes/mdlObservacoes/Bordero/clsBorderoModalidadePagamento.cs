...
{
	/// <summary>
	/// Summary description for clsBorderoModalidadePagamento.
	/// </summary>
	public class clsBorderoModalidadePagamento : clsObservacoes
	{
		#region Atributos
		...
		public clsBorderoModalidadePagamento(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string strIdPE): base(ref tratadorErro, ref ConnectionDB,EnderecoExecutavel,nIdExportador)
		{
			m_strIdPE = strIdPE;
			carregaTypDatSet();
			carregaDadosBD();
			m_bMostrarLabel = false;
			m_strCaption = "Esquema de Pagamento";;
		}
		#endregion
		...
		protected override void carregaDadosBDEspecifico()
		{
			try
			{
				...
				if (m_typDatSetTbBorderos.tbBorderos.Rows.Count > 0)
				{
					...
					if (!dtrwRowTbBorderos.IsmstrEsquemaPagamentoNull())
						m_strObservacoes = dtrwRowTbBorderos.mstrEsquemaPagamento;
				}
			} 
			catch (Exception err)
			{
			...
			try
			{
				...
				if (m_typDatSetTbBorderos.tbBorderos.Rows.Count > 0)
				{
					...
					if (dtrwRowTbBorderos != null)
						dtrwRowTbBorderos.mstrEsquemaPagamento = m_strObservacoes;
				}
