...
{
	/// <summary>
	/// Summary description for clsInstrucoesEmbarqueProcedimentos.
	/// </summary>
	public class clsInstrucoesEmbarqueProcedimentos : clsObservacoes
	{
		#region Atributos
		...
		public clsInstrucoesEmbarqueProcedimentos(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string strIdPE): base(ref tratadorErro, ref ConnectionDB,EnderecoExecutavel,nIdExportador)
		{
			m_strIdPE = strIdPE;
			carregaTypDatSet();
			carregaDadosBD();
			m_bMostrarLabel = false;
			m_strCaption = "Procedimentos no Embarque";
		}
		#endregion
		...
		protected override void carregaDadosBDEspecifico()
		{
			try
			{
				...
				if (m_typDatSetTbInstrucoesEmbarque.tbInstrucoesEmbarque.Rows.Count > 0)
				{
					...
					if (!dtrwRowTbInstrucoesEmbarque.IsmstrInstrucoesProcedimentoNull())
						m_strObservacoes = dtrwRowTbInstrucoesEmbarque.mstrInstrucoesProcedimento;
				}
			} 
			catch (Exception err)
			{
			...
			try
			{
				...
				if (m_typDatSetTbInstrucoesEmbarque.tbInstrucoesEmbarque.Rows.Count > 0)
				{
					...
					if (dtrwRowTbInstrucoesEmbarque != null)
						dtrwRowTbInstrucoesEmbarque.mstrInstrucoesProcedimento = m_strObservacoes;
				}
