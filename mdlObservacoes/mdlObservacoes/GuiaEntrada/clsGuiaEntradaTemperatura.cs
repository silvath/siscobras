...
{
	/// <summary>
	/// Summary description for clsGuiaEntradaTemperatura.
	/// </summary>
	public class clsGuiaEntradaTemperatura : clsObservacoes
	{
		#region Atributes
		...
		#region Constructors and Destructors
			public clsGuiaEntradaTemperatura(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string strIdPE): base(ref tratadorErro, ref ConnectionDB,EnderecoExecutavel,nIdExportador)
			{
				this.Multiline = false;
				this.TextForm = "Temperatura";
				m_strIdPE = strIdPE;
				carregaTypDatSet();
				carregaDadosBD();
			}
			...
			protected override void carregaDadosBDEspecifico()
			{
				try
				{
					if (m_typDatSetGuiasEntradas.tbGuiasEntrada.Rows.Count > 0)
					{
						...
						if (!dtrwGuiasEntrada.IsmstrTemperaturaNull())
							m_strObservacoes = dtrwGuiasEntrada.mstrTemperatura;
					}
				} 
				catch (Exception err)
				{
		...
		protected override void salvaDadosBDEspecifico()
		{
			if (m_typDatSetGuiasEntradas.tbGuiasEntrada.Rows.Count > 0)
			{
				...
				dtrwGuiaEntrada.mstrTemperatura = m_strObservacoes;  
