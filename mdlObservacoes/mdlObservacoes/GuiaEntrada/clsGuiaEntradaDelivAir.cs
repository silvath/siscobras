...
{
	/// <summary>
	/// Summary description for clsGuiaEntradaDelivAir.
	/// </summary>
	public class clsGuiaEntradaDelivAir : clsObservacoes
	{
		#region Atributes
		...
		#region Constructors and Destructors
			public clsGuiaEntradaDelivAir(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string strIdPE): base(ref tratadorErro, ref ConnectionDB,EnderecoExecutavel,nIdExportador)
			{
				this.Multiline = false;
				this.TextForm = "Deliv. Air";
				m_strIdPE = strIdPE;
				carregaTypDatSet();
				carregaDadosBD();
			}
			...
			protected override void carregaDadosBDEspecifico()
			{
				try
				{
					if (m_typDatSetGuiasEntradas.tbGuiasEntrada.Rows.Count > 0)
					{
						...
						if (!dtrwGuiasEntrada.IsmstrDelivAirNull())
							m_strObservacoes = dtrwGuiasEntrada.mstrDelivAir;
					}
				} 
				catch (Exception err)
				{
		...
		protected override void salvaDadosBDEspecifico()
		{
			if (m_typDatSetGuiasEntradas.tbGuiasEntrada.Rows.Count > 0)
			{
				...
				dtrwGuiaEntrada.mstrDelivAir = m_strObservacoes;  
