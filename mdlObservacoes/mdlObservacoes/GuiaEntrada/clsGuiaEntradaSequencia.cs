...
{
	/// <summary>
	/// Summary description for clsGuiaEntradaSequencia.
	/// </summary>
	public class clsGuiaEntradaSequencia : clsObservacoes
	{
		#region Atributes
		...
		#region Constructors and Destructors
			public clsGuiaEntradaSequencia(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string strIdPE): base(ref tratadorErro, ref ConnectionDB,EnderecoExecutavel,nIdExportador)
			{
				this.Multiline = false;
				this.TextForm = "Sequência";
				m_strIdPE = strIdPE;
				carregaTypDatSet();
				carregaDadosBD();
			}
			...
			protected override void carregaDadosBDEspecifico()
			{
				try
				{
					if (m_typDatSetGuiasEntradas.tbGuiasEntrada.Rows.Count > 0)
					{
						...
						if (!dtrwGuiasEntrada.IsmstrSequenciaNull())
							m_strObservacoes = dtrwGuiasEntrada.mstrSequencia;
					}
				} 
				catch (Exception err)
				{
		...
		protected override void salvaDadosBDEspecifico()
		{
			if (m_typDatSetGuiasEntradas.tbGuiasEntrada.Rows.Count > 0)
			{
				...
				dtrwGuiaEntrada.mstrSequencia = m_strObservacoes;  
