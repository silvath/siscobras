...
{
	/// <summary>
	/// Summary description for clsTrocaCondicaoAVista.
	/// </summary>
	public class clsTrocaCondicaoAVista : clsTrocaBooleano
	{
		#region Atributos
		...
		public clsTrocaCondicaoAVista(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB, string EnderecoExecutavel, int idExportador, string strIdPE) : base(ref tratadorErro, ref ConnectionDB, EnderecoExecutavel)
		{
				...
				if (m_typDatSetTbFaturasComerciais != null)
				{
					if (m_typDatSetTbFaturasComerciais.tbFaturasComerciais.Rows.Count > 0)
					{
						...
						if (!dtrwTbFaturasComerciais.IscondAvistaNull())
							m_bValorBooleano = (dtrwTbFaturasComerciais.condAvista > 0 ? true : false);
					}
				}
			}
			catch (Exception err)
