...
namespace mdlNumero.PEs
{
	...
	public class clsSD : clsNumero
	{
		...
		public clsSD(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string idPE): base(ref tratadorErro, ref ConnectionDB, EnderecoExecutavel, nIdExportador)
		{
			m_strIdPE = idPE;
			carregaTypDatSet();
			carregaDadosBD();
			...
			m_enumMascara = frmFNumero.MASCARAS.SD;
		}
		#endregion
		...
		protected override void carregaDadosBDEspecificos()
		{
			try
			{
				...
				if (m_typDatSetTbPEs.tbPEs.Rows.Count > 0)
				{
					...
					{
						if (!dtrwRowTbPEs.IsmstrSDNull())
						{
							m_strNumero = dtrwRowTbPEs.mstrSD.Replace("\0","");
							m_strNumeroNovo = dtrwRowTbPEs.mstrSD.Replace("\0","");
						}
					}
				}
			}
			...
			try
			{
				...
				if (m_strNumero != m_strNumeroNovo)
				{
					if (m_typDatSetTbPEs.tbPEs.Rows.Count > 0)
					{
						...
						if (dtrwRowTbPEs != null)
						{
							dtrwRowTbPEs.mstrSD = m_strNumeroNovo;
