...
namespace mdlNumero.PEs
{
	...
	public class clsDSE : clsNumero
	{
		...
		public clsDSE(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string idPE): base(ref tratadorErro, ref ConnectionDB, EnderecoExecutavel, nIdExportador)
		{
			m_strIdPE = idPE;
			carregaTypDatSet();
			carregaDadosBD();
			...
			m_bMascaraPEs = true;
			m_strMascaraPES = "NNNNNNNNNN/N";
			m_enumMascara = frmFNumero.MASCARAS.DSE;
		}
		#endregion
		...
		protected override void carregaDadosBDEspecificos()
		{
			try
			{
				...
				if (m_typDatSetTbPEs.tbPEs.Rows.Count > 0)
				{
					...
					{
						if (!dtrwRowTbPEs.IsmstrDSENull())
						{
							m_strNumero = dtrwRowTbPEs.mstrDSE.Replace("\0","");
							m_strNumeroNovo = dtrwRowTbPEs.mstrDSE.Replace("\0","");
						}
					}
				}
			}
			...
			try
			{
				...
				if (m_strNumero != m_strNumeroNovo)
				{
					if (m_typDatSetTbPEs.tbPEs.Rows.Count > 0)
					{
						...
						if (dtrwRowTbPEs != null)
						{
							dtrwRowTbPEs.mstrDSE = m_strNumeroNovo;
							dtrwRowTbPEs.mstrRE = "";
							dtrwRowTbPEs.mstrSD = "";
						}
