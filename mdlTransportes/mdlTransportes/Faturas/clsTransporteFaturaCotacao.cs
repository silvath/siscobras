...
{
	/// <summary>
	/// Summary description for clsTransporteFaturaCotacao.
	/// </summary>
	public class clsTransporteFaturaCotacao : clsTransporte
	{
		#region Atributos
		...
		public clsTransporteFaturaCotacao(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string strIdCotacao): base(ref tratadorErro, ref ConnectionDB,EnderecoExecutavel,nIdExportador)
		{
		...
		protected override void carregaDadosBDEspecifico()
		{
			try
			{
				...
				if (m_typDatSetTbFaturasCotacoes.tbFaturasCotacoes.Rows.Count > 0)
				{
					...
					if (!dtrwRowTbFaturasCotacoes.IsnavioNull())
						m_strNavio = dtrwRowTbFaturasCotacoes.navio;
				}
			} 
			catch (Exception err)
			{
			...
			try
			{
				...
				if (m_typDatSetTbFaturasCotacoes.tbFaturasCotacoes.Rows.Count > 0)
				{
					...
					if (dtrwRowTbFaturasCotacoes != null)
						dtrwRowTbFaturasCotacoes.navio = m_strNavio;
				}
