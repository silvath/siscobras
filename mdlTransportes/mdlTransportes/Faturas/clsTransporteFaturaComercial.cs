...
{
	/// <summary>
	/// Summary description for clsTransporteFaturaComercial.
	/// </summary>
	public class clsTransporteFaturaComercial : clsTransporte
	{
		#region Atributos
		...
		public clsTransporteFaturaComercial(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string strIdPE): base(ref tratadorErro, ref ConnectionDB,EnderecoExecutavel,nIdExportador)
		{
			m_strIdPE = strIdPE;
			carregaTypDatSet();
			carregaDadosBD();
		...
		protected override void carregaDadosBDEspecifico()
		{
			try
			{
				...
				if (m_typDatSetTbFaturasComerciais.tbFaturasComerciais.Rows.Count > 0)
				{
					...
					if (!dtrwRowTbFaturasComerciais.IsnavioNull())
						m_strNavio = dtrwRowTbFaturasComerciais.navio;
				}
			} 
			catch (Exception err)
			{
			...
			try
			{
				...
				if (m_typDatSetTbFaturasComerciais.tbFaturasComerciais.Rows.Count > 0)
				{
					...
					if (dtrwRowTbFaturasComerciais != null)
						dtrwRowTbFaturasComerciais.navio = m_strNavio;
				}
