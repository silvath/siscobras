...
{
	#region AbstractSDReport
	public abstract class AbstractSDReport:Component
	{
	}
	#endregion
	...
	public class PdfDoc
	{
		...
		bool FHasDoc;
		PdfCanvas FCanvas;
		ushort FDefaultPageWidth;
		ushort FDefaultPageHeight;
		PdfInfo FInfo;
		PdfCatalog FRoot;
		bool FUseOutlines;
		...
		public ushort DefaultPageWidth
		{
			get
			{
		...
		public ushort DefaultPageHeight
		{
			get
			{
		...
		public void AddXObject(string AName,PdfXObject AXObject)
		{
			...
			if((AXObject==null)||(AXObject.Attributes==null)||
				(Generic._GetTypeOf(AXObject.Attributes)!="XObject")||
		...
		public void RegisterXobject(PdfXObject AObject,string AName)
		{
			...
			if(Generic._GetTypeOf(AObject.Attributes)!="XObject")
		...
		protected PdfFont CreateFont(string FontName)
		{
			...
			if (bIsType1Font(FontName))
			{
				pdffont=FindType1Font(FontName);
				pdffont.Data.AddItem("Name",new PdfName("F"+s));
				FFontList.Add(pdffont);
				return pdffont;
			}else{
				// TrueType Font
				pdffont = new PdfFonts.PdfTrueTypeFont(FXRef,FontName);
				pdffont.Data.AddItem("Name",new PdfName("F"+s));
				FFontList.Add(pdffont);
				return(pdffont);
			}
		...
		public bool bIsType1Font(string s)
		{		
		...
		public PdfFont FindType1Font(string s)
		{
			if(s=="Arial")
		...
		public void AddPage()
		{
			...
			Generic._Pages_AddKids(FCurrentPages,FPage);
		...
		protected PdfDictionary CreatePages(PdfDictionary Parent)
		{
			...
			if((Parent!=null)&&(Generic._GetTypeOf(Parent)=="Pages"))
				Generic._Pages_AddKids(Parent,ADictionary);
			else
	...
	{
		#region Static
			internal static bool IsUnicodeText(string strText)
			{
				if (strText == null)
					return(false);
			...
			private static string strFormatTextoToPDFText(string strText)
			{
				System.Text.StringBuilder strbText = new StringBuilder();
				for(int i = 0; (i < strText.Length);i++)
					switch(strText[i])
					{
						case '€': 
		...
		public void DrawXObject(float X,float Y,float AWidth,float AHeight,string AXObjectName)
		{
			...
			FXObjectList=Generic._Page_GetResources(FPage,"XObject");
		...
		{
			string S;
			S=Generic._FloatToStrR(a)+" "+
				Generic._FloatToStrR(b)+" "+
				Generic._FloatToStrR(c)+" "+
				Generic._FloatToStrR(d)+" "+
				Generic._FloatToStrR(e)+" "+
				Generic._FloatToStrR(f)+" cm"+(char)10;
			WriteString(S);
		}
		...
		{
			string S;
			S=Generic._FloatToStrR(X)+" "+
				Generic._FloatToStrR(Y)+" "+
				Generic._FloatToStrR(Width)+" "+
				Generic._FloatToStrR(Height)+" re"+(char)10;
			WriteString(S);
		}

		public void SetHorizantalScaling(ushort hScaling)
		{
		...
		public void SetTextRise(ushort rise)
		{
		...
		{
			string S;
			S=Generic._FloatToStrR(x)+" "+Generic._FloatToStrR(y)+" m"+(char)10;
			WriteString(S);
		}
		...
		{
			string S;
			S=Generic._FloatToStrR(x)+" "+Generic._FloatToStrR(y)+" l"+(char)10;
			WriteString(S);
		}
		...
		{
			string S;
			S=Generic._FloatToStrR(tx)+" "+
				Generic._FloatToStrR(ty)+" Td"+(char)10;
			WriteString(S);
		}
		...
		{
			string FString;
			FString="("+Generic._EscapeText(S)+")";
			FString = strFormatTextoToPDFText(FString);
		...
		public void SetLeading(float leading)
		{
			if(FAttr.Leading==leading)
				return;
			WriteString(Generic._FloatToStrR(leading)+" TL"+(char)10);
		}
		...
		{
			string S;
			S=Generic._FloatToStrR(x1)+" "+
				Generic._FloatToStrR(y1)+" "+
				Generic._FloatToStrR(x2)+" "+
				Generic._FloatToStrR(y2)+" "+
				Generic._FloatToStrR(x3)+" "+
				Generic._FloatToStrR(y3)+" c"+(char)10;
			WriteString(S);
		}
		...
		public void SetCharSpace(float charSpace)
		{
			...
			if(Contents!=null)
				WriteString(Generic._FloatToStrR(charSpace)+" Tc"+(char)10);
		}
		...
		{
			string S;
			S=Generic._FloatToStrR(linewidth)+" w"+(char)10;
			WriteString(S);
		}
		...
		string GetColorStr(Color Color)
		{
			...
			return Generic._FloatToStrR(x0)+" "+Generic._FloatToStrR(x1)+" "+
				Generic._FloatToStrR(x2);
		}
		...
		public void SetWordSpace(float wordSpace)
		{
			...
			if(Contents!=null)
				WriteString(Generic._FloatToStrR(wordSpace)+" Tw"+(char)10);
		}
		...
		public void SetFont(string AName,float ASize)
		{
			...
			if((FAttr.Font==FFont)&&(FAttr.FontSize==ASize))
				return;
			FFontList=Generic._Page_GetResources(FPage,"Font");
		...
		private static string UnicodeToUTF8(string strText)
		{
			int num7;
			int num8;
			byte[] numArray4 = Encoding.Unicode.GetBytes(strText);
		...
		/// SetFont: Teste
		/// </summary>
		/// <param name="fntFont"></param>
		public void SetFont(System.Drawing.Font fntFont,string strText)
		{
			//TODO: Insert unicode Text;

			float ASize = fntFont.Size;
			PdfFont FFont;
			PdfDictionary FFontList;
			string FFontName = fntFont.Name;
			if (fntFont.Bold)
				if (fntFont.Italic)
					FFontName = FFontName + ",BoldItalic";
				else
					FFontName = FFontName + ",Bold";
			else
				if (fntFont.Italic)
					FFontName = FFontName + ",Italic";
			FFont=FPdfDoc.GetFont(FFontName);
			if((FAttr.Font==FFont)&&(FAttr.FontSize==ASize))
				return;
			FFontList=Generic._Page_GetResources(FPage,"Font");
		...
		{
			string S;
			S=fontname+" "+Generic._FloatToStrR(size)+" Tf"+(char)10;
			WriteString(S);
		}
		...
		private void WriteString(string S)
		{
			if((!FIsVirtual)&&(FContents!=null))
				Generic._WriteString(S,FContents.Stream);
		}
		...
		public void SetPage(PdfDictionary APage)
		{
			...
			if(AFont!=null)
			{
				...
				FAttr.HorizantalScaling=(ushort)FPage.PdfNumberByName("_HScalling").Value;
	...
	public class PdfCanvasAttribute
	{
		...
		public ushort HorizantalScaling
		{
			get
			{
		...
		public DateTime CreationDate
		{
			get
			{
				if(FData.ValueByName("CreationDate")!=null)
				{
					try
					{
						return Generic._PdfDateToDateTime(FData.PdfTextByName("CreationDate").Value);
					}
					catch
					{
			...
			set
			{
				FData.AddItem("CreationDate",new PdfText(Generic._DateTimeToPdfDate(value)));
			}
		}
		...
		public DateTime ModDate
		{
			get
			{
				if(FData.ValueByName("ModDate")!=null)
				{
					try
					{
						return Generic._PdfDateToDateTime(FData.PdfTextByName("ModDate").Value);
					}
					catch
					{
						return DateTime.MinValue;
			...
			set
			{
				FData.AddItem("ModDate",new PdfText(Generic._DateTimeToPdfDate(value)));
			}
		}
	...
	public class PdfCatalog:PdfDictionaryWrapper
	{
		...
		public PdfDictionary Pages
		{
			...
			set
			{
				if(Generic._GetTypeOf(value)=="Pages")
	...
	public class PdfXRef:PdfObjectMgr
	{
		...
		public void WriteToStream(MemoryStream Astream)
		{
			...
			for(int i=0;i<Count;i++)
				S=S+this[i].AsString+"\r\n";
			Generic._WriteString(S,Astream);
		}
	...
	public class PdfTrailer
	{
		...
		public void WriteToStream(MemoryStream AStream)
		{
			Generic._WriteString("trailer"+"\r\n",AStream);
			FAttributes.WriteToStream(AStream);
			Generic._WriteString("\r\n"+"startxref"+"\r\n",AStream);
			Generic._WriteString(FXRefAddress.ToString()+"\r\n",AStream);
			Generic._WriteString("%%EOF"+"\r\n",AStream);
		}
		#endregion
	}
	#endregion
