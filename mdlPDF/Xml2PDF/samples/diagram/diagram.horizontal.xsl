...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="item">
		<fo:table-row>
			...
			<fo:table-cell padding="0.2cm" wrap-option="no-wrap">
				<fo:block>
					<fo:inline margin-right="5pt" background-color="{color}" line-height="11pt"><fo:leader leader-length="{value}*0.1cm" leader-pattern="space"/></fo:inline><xsl:value-of select="value"/>%
				</fo:block>
			</fo:table-cell>								
		</fo:table-row>
	</xsl:template>
