...
namespace mdlComponentesGraficos
{
	...
	public class TaskbarNotifier : System.Windows.Forms.Form
	{
		...
		protected Rectangle RealContent2Rectangle;
		...
		protected Color normalContent2Color = Color.FromArgb(0,0,0);
		protected Color hoverContentColor = Color.FromArgb(0,0,0x66);
		protected Color hoverContent2Color = Color.FromArgb(0,0,0x66);
		...
		protected Font normalContent2Font = new Font("Arial",11,FontStyle.Regular,GraphicsUnit.Pixel);
		protected Font hoverContent2Font = new Font("Arial",11,FontStyle.Regular,GraphicsUnit.Pixel);
		...
		protected bool bIsMouseOverContent2 = false;
		...
		public Rectangle Content2Rectangle;
		public bool TitleClickable = false;
		public bool ContentClickable = true;
		public bool Content2Clickable = true;
		...
		public event EventHandler Content2Click = null;
		#endregion
		...
		/// Get/Set the Normal Content Color
		/// </summary>
		public Color NormalContent2Color
		{
			get
			{
				return normalContent2Color;
			}
			set
			{
				normalContent2Color = value;
				Refresh();
			}
		}
		...
		/// Get/Set the Hover Content Color
		/// </summary>
		public Color HoverContent2Color
		{
			get
			{
				return hoverContent2Color;
			}
			set
			{
				hoverContent2Color = value;
				Refresh();
			}
		}
		...
		/// Get/Set the Normal Content Font
		/// </summary>
		public Font NormalContent2Font
		{
			get
			{
				return normalContent2Font;
			}
			set
			{
				normalContent2Font = value;
				Refresh();
			}
		}
		...
		/// Get/Set the Hover Content Font
		/// </summary>
		public Font HoverContent2Font
		{
			get
			{
				return hoverContent2Font;
			}
			set
			{
				hoverContent2Font = value;
				Refresh();
			}
		}
		...
		{
			Show(strTitle,strContent,"",nTimeToShow,nTimeToStay,nTimeToHide);
		}
		...
		/// 
		public void Show(string strTitle, string strContent,string strContent2, int nTimeToShow, int nTimeToStay, int nTimeToHide)
		{
			WorkAreaRectangle = Screen.GetWorkingArea(WorkAreaRectangle);
			titleText = strTitle;
			contentText = strContent;
			content2Text = strContent2;
		...
		protected void DrawText(Graphics grfx)
		{
			...
			if (content2Text != null && content2Text.Length != 0)
			{
				StringFormat sf = new StringFormat();
				sf.Alignment = StringAlignment.Center;
				sf.LineAlignment = StringAlignment.Center;
				sf.FormatFlags = StringFormatFlags.MeasureTrailingSpaces;
				sf.Trimming = StringTrimming.Word;							// Added Rev 002
				if (bIsMouseOverContent2)
				{
					grfx.DrawString(content2Text, hoverContent2Font, new SolidBrush(hoverContent2Color), Content2Rectangle, sf);
					if (EnableSelectionRectangle)
						ControlPaint.DrawBorder3D(grfx, RealContent2Rectangle, Border3DStyle.Etched, Border3DSide.Top | Border3DSide.Bottom | Border3DSide.Left | Border3DSide.Right);
					
				}
				else
					grfx.DrawString(content2Text, normalContent2Font, new SolidBrush(normalContent2Color), Content2Rectangle, sf);
			}
		}
		...
		protected void CalculateMouseRectangles()
		{
			...
			SizeF sizefContent2 = grfx.MeasureString(content2Text, hoverContent2Font, Content2Rectangle.Width, sf);
			grfx.Dispose();
			...
			// Added Rev 002
			//We should check if the Content size really fits inside the pre-defined Content rectangle
			if (sizefContent2.Height > Content2Rectangle.Height)
			{
				RealContent2Rectangle = new Rectangle((Content2Rectangle.Width-(int)sizefContent2.Width)/2+Content2Rectangle.Left, Content2Rectangle.Top, (int)sizefContent2.Width, Content2Rectangle.Height );
			}
			else
			{
				RealContent2Rectangle = new Rectangle((Content2Rectangle.Width-(int)sizefContent2.Width)/2+Content2Rectangle.Left, (Content2Rectangle.Height-(int)sizefContent2.Height)/2+Content2Rectangle.Top, (int)sizefContent2.Width, (int)sizefContent2.Height);
			}
			RealContent2Rectangle.Inflate(0,2);
		}
		...
		protected override void OnMouseMove(MouseEventArgs mea)
		{
			...
			else if (RealContent2Rectangle.Contains(new Point(mea.X, mea.Y)) && Content2Clickable)
			{
				if (!bIsMouseOverContent2)
				{
					bIsMouseOverClose = false;
		...
		protected override void OnMouseUp(MouseEventArgs mea)
		{
			...
			else if (bIsMouseOverContent2)
			{
				if (Content2Click != null)
					Content2Click(this, new EventArgs());
			}
		}
