...
{
	/// <summary>
	/// Summary description for clsNumeroOrdemCompraComercial.
	/// </summary>
	public class clsNumeroOrdemCompraComercial : clsNumeroOrdemCompra
	{
		#region Atributos
		...
		public clsNumeroOrdemCompraComercial(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string strIdPE): base(ref tratadorErro, ref ConnectionDB,EnderecoExecutavel,nIdExportador)
		{
			m_strIdPE = strIdPE;
			carregaTypDatSet();
			carregaDadosBD();
		...
		protected override void carregaDadosBDEspecifico()
		{
			try
			{
				...
				if (m_typDatSetTbFaturasComerciais.tbFaturasComerciais.Rows.Count > 0)
				{
					...
					if (!dtrwRowTbFaturasComerciais.IsordemCompraNull())
						m_strNumeroOrdemCompra = dtrwRowTbFaturasComerciais.ordemCompra;
				}
			} 
			catch (Exception err)
			{
			...
			try
			{
				...
				if (m_typDatSetTbFaturasComerciais.tbFaturasComerciais.Rows.Count > 0)
				{
					...
					if (dtrwRowTbFaturasComerciais != null)
						dtrwRowTbFaturasComerciais.ordemCompra = m_strNumeroOrdemCompra;
				}
