...
{
	/// <summary>
	/// Summary description for clsNumeroOrdemCompraProforma.
	/// </summary>
	public class clsNumeroOrdemCompraProforma : clsNumeroOrdemCompra
	{
		#region Atributos
		...
		public clsNumeroOrdemCompraProforma(ref mdlTratamentoErro.clsTratamentoErro tratadorErro, ref mdlDataBaseAccess.clsDataBaseAccess ConnectionDB,string EnderecoExecutavel,int nIdExportador, string strIdPE): base(ref tratadorErro, ref ConnectionDB,EnderecoExecutavel,nIdExportador)
		{
			m_strIdPE = strIdPE;
			carregaTypDatSet();
			carregaDadosBD();
		...
		protected override void carregaDadosBDEspecifico()
		{
			try
			{
				...
				if (m_typDatSetTbFaturasProformas.tbFaturasProformas.Rows.Count > 0)
				{
					...
					if (!dtrwRowTbFaturasProformas.IsordemCompraNull())
						m_strNumeroOrdemCompra = dtrwRowTbFaturasProformas.ordemCompra;
				}
			} 
			catch (Exception err)
			{
			...
			try
			{
				...
				if (m_typDatSetTbFaturasProformas.tbFaturasProformas.Rows.Count > 0)
				{
					...
					if (dtrwRowTbFaturasProformas != null)
						dtrwRowTbFaturasProformas.ordemCompra = m_strNumeroOrdemCompra;
				}
