...
namespace mdlModem
{
	...
	internal class clsSerialStream : Stream 
	{
		...
		public override long Length 
		{
			get 
			{
				throw new NotSupportedException("Propriedade Length não suportado em portas seriais.");
			}
		}
		...
		public override long Position 
		{
			get 
			{ 
				throw new NotSupportedException("Propriedade posição não suportada em portas seriais.");
			}
			set 
			{
				throw new NotSupportedException("Propriedade posição não suportada em portas seriais.");
		...
		public void Open(string port) 
		{
			...
			{
				throw new IOException("Stream já aberto.");
			}
			m_sPort = port;
			...
			{
				m_hFile = IntPtr.Zero;
				throw new FileNotFoundException("Não foi possível abrir " + port);
			}
		...
		public override IAsyncResult BeginRead(byte[] buffer, int offset, int count, AsyncCallback callback, object state) 
		{
			...
			unsafe 
			{
				...
				else if (GetLastError() == ERROR_IO_PENDING) 
				{
					...
					throw new Exception("Não foi possível inicializar leitura. Código de erro: " + GetLastError().ToString());
			}
		}
		...
		{
			throw new NotSupportedException("SetLength não suportado em portas seriais.");
		}

		public override long Seek(long offset, SeekOrigin origin) 
		{
			throw new NotSupportedException("Seek não suportado em portas seriais.");
		}
		...
		private const uint PURGE_TXABORT = 0x0001;  // Destrói escritas pendentes/correntas na porta COM.
		private const uint PURGE_RXABORT = 0x0002;  // Destrói leituras pendentes/correntas na porta COM.
		private const uint PURGE_TXCLEAR = 0x0004;  // Destrói fila de transmissão caso exista.
		private const uint PURGE_RXCLEAR = 0x0008;  // Destrói typeahead buffer caso exista.

		private const uint SETXOFF  = 1;	// Simula recepção XOFF.
		private const uint SETXON   = 2;	// Simula recepção XON.
